...
function AdminBadge(props: { item: Membership }): JSX.Element {
  ...
  if (item.adminRequest) return <HelpIcon color="warning" />;
  return <></>;
}
    ...
    <TableRow component={DraggableComponent(item.dragId, index)}>
      ...
      <TableCell>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          <Avatar src={item.student.picture} alt={item.student.name} size="m" />
      ...
      <TableCell>
        <Box sx={{ display: 'flex', gap: 1 }}>
          ...
          <IconButton
            ...
            hidden={group.groupType.noMembershipDates || item.endDate < today}
          >
