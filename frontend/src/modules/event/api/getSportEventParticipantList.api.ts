...
interface GetSportEventParticipantListOptions {
  page?: number;
  pageSize?: number;
}

export async function getSportEventParticipantListApi(
  eventId: number,
  options: GetSportEventParticipantListOptions,
): Promise<Page<UserPreview>> {
  const { data } = await axios
    .get<PageDTO<UserPreviewDTO>>(`/api/event/sport/${eventId}/participants/`, {
      params: {
        page: options.page,
