...
interface GetSportEventNonParticipantListOptions {
  page?: number;
  pageSize?: number;
}

export async function getSportEventNonParticipantListApi(
  eventId: number,
  options: GetSportEventNonParticipantListOptions,
): Promise<Page<UserPreview>> {
  const { data } = await axios
    .get<PageDTO<UserPreviewDTO>>(
      `/api/event/sport/${eventId}/non_participants/`,
      {
        params: {
          page: options.page,
