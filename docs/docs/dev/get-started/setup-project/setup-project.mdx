...
For a more detailed version, visit the [docker page](/server/staging.mdx).

If you have docker installed, you may want to use it to develop the project.
To set up the containers, just run `make docker-install` to initialize the project.
Then, in order to start the project, just run `make docker-start`. Yau may
proceed with regular setup instruction from now on.
The server is running at [http://localhost](http://localhost), and the fake
...
If you use an IDE that supports devcontainers, you can open the project with it.
It allows you to have a clean isolated setup at the cost of a slightly higher
ressources usage.
