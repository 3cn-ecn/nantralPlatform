...
3. Install dependencies and create a virtual environment for python:
...
5. Create an administrator account on this database:
   ...
   _Note: if `admin` already exists, run `uv run manage.py changepassword admin`
...
1. Scroll down to the **_Permissions_** section, and check the 3 checkboxes:
   ...
   - `Verified email` (next to your email at the bottom of the page): this allows you to log in with this new account!
...
For a more detailed version, visit the [docker page](../../server/docker.mdx).

If you have docker installed, you may want to use it to develop the project.
This allows to have a clean isolated environment at the cost of slightly higher
ressources usage. To set up the containers, just run `make docker-install` to
initialize the project. Then, in order to start the project, run `make docker-start`.
You may proceed with regular setup instruction [(admin account)](#create-your-admin-account)
from now on. The server is running at [http://localhost](http://localhost), and the fake
mailserver at [http://localhost:8025](http://localhost:8025)

Some useful commands:

- Change directory to the docker root: `cd deployment` (needed for the following commands)
- Open a terminal in the container: `docker compose exec -it backend-develop bash` (first start the container)
- Manually start / shutdown docker: `docker compose up --watch` and `docker compose down`
