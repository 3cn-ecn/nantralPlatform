...
-  Backups are made every day at 05:00 AM and each time you push an update to the server.
...
1. Copy the backup from the server to the Postgres docker container:
   ```bash
   sudo docker cp backup.sql deployment_database_1:dump.sql
   ```
...
1. Replace the current database with the backup:
   ```bash
   psql -U <db_username> <db_name> < dump.sql
   ```
