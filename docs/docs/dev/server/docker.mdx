...
:::warning Temporary docker compose migration
Docker compose does not work at the moment for local development. Please see the status of the `.old` files in `deployment/`.
:::

## Why use Docker on local development?
...
These services are launched when you run Docker on your local machine:
...
:::note **Switching to `docker compose`**
We used to run our compose files with `docker-compose` or the `dcf` alias on our server. We now use `docker compose`, and `docker-compose` will throw a command not found error.

A great explanation of the difference between the two can be found [here](https://stackoverflow.com/a/66516826).
:::
