...
- **Use the cache system**: Django comes with a caching system, that you can
...
6. Start the services with source synchronization enabled: `docker compose up --watch`
...
| ----------------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| database          | The postgresql database for the website                 | You can read it by connecting to the port 5432 on localhost with [PgAdmin4](http://localhost:5050/)                 |
| backend           | The django server which serve files                     | Open [http://localhost](http://localhost) in your browser                                                           |
| memcached         | Used for the django cache system                        | -                                                                                                                   |
| redis             | Used for the celery brocker                             | -                                                                                                                   |
...
| PG Admin 4        | A client to connect to the database and run SQL queries | Open [http://localhost:5050](http://localhost:5050), then add a database with the credentials from your `.env` file |
...
2. Create the environment files at the root of this directory:
   - `backend.env` for all environment variables related to the server
   - `backend-staging.env` for all environment variables related to the staging server
   - `.env` for docker injection (used for docker images tags)
3. Chose the tags (commit sha or `latest`) that you want to use for the backend and the staging images
   Put these values in the `.env` file, then pull the images for GitHub:

   ```bash
   docker compose -f docker-compose.yml -f docker-compose.prod.yml pull
   ```

4. Build and run the docker compose files:
   ```bash
