...
If you need more information, re-run the build directly on the server:

1. Connect to the server using SSH.
2. Go to nantralPlatform/deployment by running `cd nantralPlatform/deployment`.
3. Run `docker build -f deployment/containers/web/Dockerfile .` to re-build the containers.

:::info
In order to change the running container, you will need to push it to ghcr.io (GitHub repository for docker images).
Once you've done this, update the version tag in the `.env` file with the new tag, then run `dcf pull && dcf up`.
