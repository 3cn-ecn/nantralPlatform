...
3. Run `sudo docker compose build` to re-build the containers.
...
1. Connect to the server using SSH.
2. Go to nantralPlatform/deployment by running `cd nantralPlatform/deployment`.
3. Run `docker compose logs --follow`.
4. Press CTRL+C to exit.

If the containers are not running, use: `docker compose up` to bring them online.

Once you are done debugging, press CTRL+C to exit and then run `docker compose down && docker compose up -d` to bring the containers back online in detached mode.
...
1. Connect to the server using SSH.
2. Run `docker compose logs backend --follow`.
