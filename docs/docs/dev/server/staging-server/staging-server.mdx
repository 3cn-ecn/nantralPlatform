...
- When you update the database (for example creating new migrations or updating
  the `models.py` files)
- When you add new dependencies to the project (`pyproject.toml` or `package.json`)
...
- all modifications of the email templates
- the `Dockerfile` (only the `production` image)

You **_cannot_** test:

- the documentation website
- the periodic celery tasks (scheduled tasks)
- the docker configuration (`docker-compose*.yml`, `stack/*.yml`)
...
:::info
First, try to run your code on your machine with docker. If the code does not work there,
it won't work on the server either.
:::
