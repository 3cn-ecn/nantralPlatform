...
Vous pouvez ajouter de nouvelles versions de vos évènements, traduits dans
différentes langues de votre choix, parmis celles disponibles sur le site :

- sélectionnez la langue dans laquelle vous comptez traduire votre
  évènement dans le sélecteur en haut de l'interface
- ajoutez le titre et la description de l'évènement traduit dans la langue
  sélectionnée.

Lorsque vous naviguez entre les différentes langues, l'interface vous affiche
les traductions que vous avez rentrées pour la langue sélectionnée.

![Gif détaillant comment définir plusieurs traductions pour un évènement](./definition_trad.gif)

Une fois l'évènement publié, plus besoin de rien faire, le site s'occupe
automatiquement d'afficher la bonne traduction en fonction de la langue
sélectionnée dans les options par l'utilisateur.

Si un évènement ne comporte qu'une seule traduction, celle-ci sera utilisée,
peu importe la langue sélectionnée par l'utilisateur.
