...
def makeGroupUrlpatterns(
    ...
    '''Fonction créant toutes les urls pour un groupe (ex: clubs, listes, colocs, cours...).
       -> group_type est la première partie du slug, correspondant au type
       -> url_base est l'url qui peut être ajoutée devant les slugs
       -> *_view permet de remplacer la vue générale de group par une vue personnalisée pour le groupe '''
    
    args = {'group_type': group_type}

    urlpatterns = [
        # vue generale du groupe
        path(url_base+'<slug:mini_slug>/', detail_view, args, name='detail'),
        path(url_base+'<slug:mini_slug>/members/add/', add_member_view, args, name='add-member'),

        # edition (réservée aux admins)
        path(url_base+'<slug:mini_slug>/edit/', update_view, args, name='update'),
        path(url_base+'<slug:mini_slug>/edit/members/', update_members_view, args, name='update-members'),
        path(url_base+'<slug:mini_slug>/edit/socialnetworks/', update_sociallinks_view, args, name='update-sociallinks'),
        path(url_base+'<slug:mini_slug>/edit/events/', update_events_view, args, name='update-events'),
        path(url_base+'<slug:mini_slug>/edit/events/create/', create_event_view, args, name='create-event'),
        path(url_base+'<slug:mini_slug>/edit/events/archives/', archived_events_view, args, name='archived-events'),
        path(url_base+'<slug:mini_slug>/edit/posts/', update_posts_view, args, name='update-posts'),
        path(url_base+'<slug:mini_slug>/edit/posts/create/', create_post_view, args, name='create-post'),
