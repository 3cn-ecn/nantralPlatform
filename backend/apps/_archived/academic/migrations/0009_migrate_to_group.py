...
def forwards(apps, schema_editor):
    ...
    labels = {
        "OP": Label.objects.create(
            name="Option Professionnelle", group_type=gt
        ),
...
def reverse(apps, schema_editor):
    ...
    for group in Group.objects.filter(
        group_type__slug="academic", parent__isnull=False
    ):
        ...
        for m in group.membership_set.all():
            c.members.add(
                m.student,
                through_defaults={"admin": m.admin, "year": m.begin_date.year},
            )
        for student in group.subscribers.all():
            Subscription.objects.create(
                student=student, page=f"academic--{c.slug}"
            )
