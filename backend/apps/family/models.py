...
class Family(Group):
    ...
    non_subscribed_members = models.CharField(
        "Autres parrains", max_length=300, null=True, blank=True,
...
class MembershipFamily(NamedMembership):
    ...
    group = models.ForeignKey(
        to=Family, on_delete=models.CASCADE, null=True, blank=True,
        related_name='memberships')
    student = models.ForeignKey(
        to=Student, verbose_name="Parrain/Marraine", on_delete=models.CASCADE,
...
class BaseQuestion(models.Model):
    ...
    details_en = models.CharField(
        "Informations supplémentaires (en)", max_length=200, null=True,
        blank=True)
    order = models.IntegerField(
...
class QuestionFamily(BaseQuestion):
    equivalent = models.OneToOneField(
        to=QuestionMember, related_name='equivalent',
