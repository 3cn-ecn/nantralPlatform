...
def get_answer(member: MembershipFamily, question):
    ...
    answers = [ans for ans in member.answermember_set.all(
    ...
    if len(answers) == 1:
        ...
        if member.role == '2A+' and question['equivalent'] is not None:
            ...
            try:
                f_ans = [ans for ans in member.group.answerfamily_set.all(
                ) if ans.question.id == question['equivalent']][0]
...
def get_member_1A_list(question_list, itii=False):  # noqa: N802
    ...
    data = (
        ...
        .filter(role='1A', group__isnull=True)
        .prefetch_related(
...
def get_member_2A_list(question_list):  # noqa: N802
    ...
    data = (
        MembershipFamily.objects
        .filter(role='2A+', group__year=scholar_year())
        .prefetch_related(
            'answermember_set__question',
            'group__answerfamily_set__question'
        )
    ...
    for m in member2A_list:
        if vecthasnan(m['answers']):
            ...
            fam_answers = [f for f in family_list if f['family']
                           == m['family']][0]['answers']
...
def get_family_list(member2A_list):  # noqa: N803
    ...
    for fam in Family.objects.filter(year=scholar_year()):
        answers_list = np.array([m['answers']
                                for m in member2A_list if m['family'] == fam])
    ...
    for f in family_list:
        if f['nb_criteria_1A'] == 1 and f['nb_criteria_2A'] == 0:
            ...
            if candidate_member_list:
                ...
                candidate_member_id = [
                    i for i in range(len(member1A_list))
                    if member1A_list[i] == candidate_member
                ][0]
                candidate_family_id = [
                    i for i in range(len(family_list))
                    if family_list[i]['family'] == candidate_member['family']
                ][0]
                # on échange les familles
                member1A_list[lonely_member_id]['family'] = (
                    family_list[candidate_family_id]['family'])
